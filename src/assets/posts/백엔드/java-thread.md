...
스레드는 Runnable 인터페이스를 구현한 작업을 수행할 수 있다.
Runnable 인터페이스는 run()함수만 갖고 있다. thread.start()를 하게 되면 실제 스레드가 실행된다.

다만 스레드의 우선순위는 스ㄴ케줄링에 따라서 다르게 실행될 수 있다.
...
Thread.start()를 사용해야지만 새로운 스레드가 생성되고 새로운 스레드 안에서 run()함수를 실행해서 작업이 나뉘어 병렬적으로 처리된다.
